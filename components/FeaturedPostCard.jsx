...
const FeaturedPostCard = ({blogPost}) => {
  return (
     ...
     <p className="text-white text-shadow font-semibold md:font-light text-xs p-2">{blogPost.excerpt}</p>
     <div className="justify-end">
          {blogPost.categories.map((category, index) => (
              <Link key={category.slug} href={`/category/${category.slug}`}> 
             <span className="inline-block cursor-pointer bg-gray-200 rounded-full px-3 py-1 text-sm md:text-[8px] font-semibold text-gray-700 hover:text-pink-500 mr-2 mb-3">{category.name}</span>
        </Link>
          ))}
          </div>
     </div>
